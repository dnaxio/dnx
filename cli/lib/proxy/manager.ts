...
import { generateCaddyfile, generateSimpleCaddyfile, type CaddyRoute } from "./caddyfile.ts";
...
const CADDY_STATUS_CMD = "caddy version && pgrep caddy && echo RUNNING || echo NOT_RUNNING";
...
export async function installCaddyRemote(conn: SSHConnection): Promise<void> {
  if (await isCaddyInstalled(conn)) {
    logger.debug("Caddy déjà installé.");
    return;
  }
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Échec de l'installation de Caddy : ${result.stderr}`);
  }
  logger.success("Caddy installé.");
}
...
export async function uploadCaddyfile(
  ...
  logger.debug("Caddyfile uploadé.");
}
...
export async function reloadCaddy(conn: SSHConnection): Promise<void> {
  ...
  if (result.exitCode !== 0 && !result.stderr.includes("not running")) {
    throw new Error(`Caddy reload échoué : ${result.stderr}`);
  }
  logger.debug("Caddy rechargé.");
}
...
export async function startCaddy(conn: SSHConnection): Promise<void> {
  ...
  if (status.stdout.includes("RUNNING")) {
    logger.info("Caddy déjà en cours d'exécution.");
    return;
  }

  const result = await conn.exec(CADDY_START_CMD);
  if (result.exitCode !== 0) {
    throw new Error(`Caddy start échoué : ${result.stderr}`);
  }
  logger.success("Caddy démarré.");
}
...
export async function stopCaddy(conn: SSHConnection): Promise<void> {
  ...
  if (result.exitCode !== 0 && !result.stderr.includes("not running")) {
    throw new Error(`Caddy stop échoué : ${result.stderr}`);
  }
  logger.success("Caddy arrêté.");
}
...
export async function restartCaddy(conn: SSHConnection): Promise<void> {
  ...
  logger.success("Caddy redémarré.");
}
